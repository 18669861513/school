...
public class OrganizationDaoImpl implements OrganizationDao {
	...
	public List<Organization> findAllWithExclude(
		...
		return jdbcTemplate.query(sql, new BeanPropertyRowMapper(
				Organization.class), excludeOrganization.getId(),
				excludeOrganization.makeSelfAsParentIds() + "%");
	}

	public void move(Organization source, Organization target) {
		String moveSql = "update sys_organization set parent_id = ?,parent_ids=? where id=?";
		jdbcTemplate.update(moveSql, target.getId(), target.getParentIds(),
				source.getId());
		String moveSourceDescendantsSql = "update sys_organization set parent_id=concat(?,substring(parent_ids,length(?))) where parent_ids like ?";
